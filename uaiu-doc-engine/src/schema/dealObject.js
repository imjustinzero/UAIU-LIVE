...
const dealObjectShape = z.object({
...
function applyDealRefinements(deal, ctx) {
...
const dealObjectSchema = dealObjectShape.superRefine(applyDealRefinements);

const listingValidationSchema = dealObjectShape
  .pick({
    trade_id: true,
    listing_id: true,
  ...
  .superRefine(applyDealRefinements);
