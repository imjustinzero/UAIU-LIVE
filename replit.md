...
- **Wave 3 Exchange Features**: 6 new features fully integrated into UAIU.LIVE/X (/x route). (1) **AI Compliance CoPilot** — floating gold button (bottom-right), opens a conversational AI-powered assistant aware of ETS price, portfolio spend, account type, and available listings; calls `/api/ai/copilot`. (2) **Due Diligence Report** — AI-generated DD report per listing card (after ListingChat); calls `/api/ai/due-diligence` with listing data. (3) **AI Price Prediction Engine** — section id="prediction" after the trust section; SVG line chart showing 10-day historical + 30-day AI forecast with confidence bands; calls `/api/ai/price-prediction`. (4) **Carbon Budget Tracker** — section id="budget"; set annual budget, auto-fills session trades, shows spend-to-budget gauge, recommends cheapest listings to cover remaining liability; calls `/api/ai/budget-suggestions`. (5) **Regulatory Calendar** — section id="calendar"; EU ETS/CORSIA/IMO/FuelEU/UK ETS/CBAM deadlines for 2026+; subscribe to email reminders; calls `/api/exchange/calendar-subscribe`. (6) **Escrow Settlement** — Stripe PaymentIntent with `capture_method: manual`; triggers when trade ≥ €10,000; UI guides buyer through card entry, verify, and release steps; routes at `/api/escrow/{create,verify,release,cancel,status}`. Nav links added: Forecast, Budget, Calendar. Component files: `client/src/components/exchange/{ComplianceCoPilot,AIPredictionAndDD,CarbonBudgetTracker,RegulatoryCalendar,EscrowUI}.tsx`. Supabase tables: `escrow_settlements`, `calendar_subscriptions` in `SUPABASE-SCHEMA.sql`.
- **Wave 2 Exchange Features**: 5 new features integrated into UAIU.LIVE/X (/x route). (1) **Terminal Mode** — press T anywhere on the exchange to toggle a full-screen Bloomberg-style green-on-black terminal overlay showing live listings, trades, and market data. (2) **Voice RFQ** — mic button in the RFQ section uses Web Speech API + AI to parse spoken orders and auto-fill the RFQ form fields (side, standard, volume, price, notes). (3) **Video Trade Room** — each listing card has a "Request Live Trade Call" button that creates a Daily.co room via existing `/api/daily/create-room` endpoint and opens a meeting link. (4) **AI Trade Negotiator** — appears below the RFQ form after submission; calls `/api/ai/negotiate` (new route) which uses AI to analyze the RFQ against market conditions and returns a recommendation (action, counter_price, rationale, risk_assessment, confidence). (5) **Real-time Listing Chat** — Socket.io chat panel on each listing card; users join a per-listing room, see history, and exchange messages in real time. Socket handlers: `join-listing-chat`, `listing-chat-message`, `leave-listing-chat`. In-memory Maps: `listingChatHistory`, `listingOnlineUsers`. Component files in `client/src/components/exchange/`: TerminalMode.tsx, VoiceRFQ.tsx, TradeFeatures.tsx, ListingChat.tsx.
