...
- **Data Models**: `User`, `Match`, `ActionLog`, `PayoutRequest`, and `ReferralPayout` with clear schemas.
...
The system utilizes a client-server architecture. The server manages all critical game logic, credit transactions, and user authentication to ensure fairness. Data is stored in PostgreSQL. Socket.IO provides low-latency real-time game state synchronization. Due to Neon HTTP driver limitations, credit operations for likes/comments use sequential atomic updates instead of full database transactions, with manual rollback on failure.
