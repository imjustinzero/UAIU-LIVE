...
- **UAIU.LIVE/X Exchange**: Added a full carbon credit marketplace at /x route. Features a live listings page (6 verified credit types: EU ETS, VCS, Gold Standard, CORSIA, Blue Carbon), filterable by standard with real-time search. Includes an interactive EU ETS Compliance Cost Calculator. Sellers can submit credits via a "List Your Credits" form. Buyers open accounts via an "Open Account" form. Both forms store to the database (exchange_accounts, exchange_credit_listings tables) and email info@uaiu.live via Resend. Citizens Portal section highlights 5 Carbon Union nations. Listings seeded from PDF data on first request.
